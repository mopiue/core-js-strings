...
function findLongestWord(sentence) {
  ...
  const wordsLengthArr = sentence.split(' ').map((item) => item.length);
  return splitArr[wordsLengthArr.indexOf(Math.max(...wordsLengthArr))];
}
...
function reverseWords(str) {
  ...
  splitArr = splitArr.map((item) => item.split('').reverse().join(''));
...
function encodeToRot13(str) {
  return str
    ...
    .map((char) => {
      ...
      if (charCode < 78 || (charCode >= 97 && charCode < 110))
